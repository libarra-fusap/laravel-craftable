...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('last_login_at'), 'has-success': fields.last_login_at && fields.last_login_at.valid }">
    <label for="last_login_at" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-3'">{{ trans('admin.admin-user.columns.last_login_at') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-7'">
        <datetime v-model="form.last_login_at" :config="datetimePickerConfig" v-validate="'date_format:yyyy-MM-dd HH:mm:ss'" class="flatpickr" :class="{'form-control-danger': errors.has('last_login_at'), 'form-control-success': fields.last_login_at && fields.last_login_at.valid}" id="last_login_at" name="last_login_at" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_date_and_time') }}"></datetime>
